...
In this task we were supposed to implement a microkernel that computes C+=AB for M=64, N=64 and K=64. Recalling our ``matmul_64_48_64`` kernel, we only need to change the N dimension to 64. This kernel uses the ``matmul_16_4_64`` internally, which we changed to ``matmul_16_4_64``. Changing N from 6 to 4 allows us to block the N dimension into 16 blocks of 4 elements. N = 8 was not suitable, as we ran into issues with the number of SIMD lanes. We do not think it is necessary to show the code for this kernel, as it is very similar to the ``matmul_64_48_64`` kernel. The only difference is that we need removed the logic for 2 of the 6 columns and increased the loop counter constant.
