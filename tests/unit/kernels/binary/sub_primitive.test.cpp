...
void test_sub_primitive(uint32_t M,
                        uint32_t N)
{
    ...
    mini_jit::kernels::binary::sub(l_kernel, M, N);
...
TEST_CASE("Tests the sub primitive with different M and N", "[sub_primitive][parameterized]")
{
    ...
    test_sub_primitive(M, N);
}

TEST_CASE("Tests the sub primitive with larger M and N", "[sub_primitive][large]")
{
    uint32_t M = 64;
    uint32_t N = 65;
    test_sub_primitive(M, N);
