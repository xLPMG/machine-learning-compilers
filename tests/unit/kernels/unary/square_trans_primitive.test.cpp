...
#include <mlc/kernels/unary/square_trans_primitive.h>
#include <random>

void test_square_trans_primitive(uint32_t M,
                                 uint32_t N)
{
    ...
    for (u_int32_t i = 0; i < M; ++i)
    {
        ...
        {
            B_expected[i * N + j] = A[j * M + i] * A[j * M + i];
        }
    }

    mini_jit::Kernel l_kernel;
    mini_jit::kernels::unary::square_trans(l_kernel, M, N);
...
TEST_CASE("Tests the square trans primitive with different M and N", "[square_trans_primitive][parameterized]")
{
    uint32_t M = GENERATE(1, 2, 3, 4, 5, 6, 7, 8);
    uint32_t N = GENERATE(1, 2, 3, 4, 5, 6, 7, 8);
    test_square_trans_primitive(M, N);
}

TEST_CASE("Tests the square trans primitive with larger M and N", "[square_trans_primitive][large]")
{
    uint32_t M = 64;
    uint32_t N = 65;
    test_square_trans_primitive(M, N);
