...
void test_increment_trans_primitive(uint32_t M,
                                    uint32_t N)
{
...
TEST_CASE("Tests the increment trans primitive with different M and N", "[increment_trans_primitive][parameterized]")
{
    uint32_t M = GENERATE(1, 2, 3, 4, 5, 6, 7, 8);
    uint32_t N = GENERATE(1, 2, 3, 4, 5, 6, 7, 8);
    test_increment_trans_primitive(M, N);
}

TEST_CASE("Tests the increment trans primitive with larger M and N", "[increment_trans_primitive][large]")
{
    uint32_t M = 64;
    uint32_t N = 65;
    test_increment_trans_primitive(M, N);
